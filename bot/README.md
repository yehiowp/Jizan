...
124 條斷言，把 `gmgn-cli` 換成假的跑完整條買賣流程。不需要 API Key、不連網、不會送出任何交易。涵蓋：評分邊界、12 種陷阱是否被攔、蜜罐四層判定、深度三種取法、5 分鐘回撤硬停與降級、CLI 參數組裝（含惡意字串不被拆開）、限流/401/自動下單開關的錯誤處理、訂單輪詢必須到 `confirmed`、風控七道上限、自動停機、壞檔復原。
